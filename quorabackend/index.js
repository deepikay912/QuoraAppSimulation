...
const storagepic2 = multer.diskStorage({
 ...
 console.log("inside write answer");
 console.log(req.body.owner);
    Model.UserModel.findOne({"Email":req.body.owner},function(err,user)
    {

        if(err) 
        res.end('Unable to fetch user'+err)
        if(!user) 
        res.end('Unable to fetch user')
        if(user) {
            console.log("inside userin  answer");
        ...
        .then(response =>{
            ...
            Model.QuestionsModel.findOne({"Question":req.body.question},(err,question)=>{
                if(err) 
                res.end('Unable to fetch question'+err)
                if(!question) 
                res.end('Unable to fetch question')
                else {
